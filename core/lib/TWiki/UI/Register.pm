...
sub _requireVerification {
    ...
    $session->writeLog( 'regstart',
        $TWiki::cfg{UsersWebName} . '.' . $data->{WikiName},
...
sub resetPassword {
    ...
    unless (@userNames) {
        throw TWiki::OopsException( 'attention', def => 'no_users_to_reset' );
    }
...
sub _createUserTopic {
    ...
    if ( $store->topicExists( $TWiki::cfg{UsersWebName}, $template ) ) {
        ...
        ( $meta, $text ) =
          $store->readTopic( undef, $TWiki::cfg{UsersWebName}, $template );
    }
    else {

        # Use the default read-only version
        ( $meta, $text ) =
          $store->readTopic( undef, $TWiki::cfg{SystemWebName}, $template );
    }
...
sub _buildConfirmationEmail {
    ...
    $templateText =
      $session->handleCommonTags( $templateText, $TWiki::cfg{UsersWebName},
        $data->{WikiName} );
...
sub _sendEmail {
    ...
    $text = $session->handleCommonTags( $text, $TWiki::cfg{UsersWebName},
        $p->{WikiName} );
