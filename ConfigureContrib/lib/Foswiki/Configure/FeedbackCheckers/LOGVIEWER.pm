...
package Foswiki::Configure::FeedbackCheckers::LOGVIEWER;

=begin TML

---+ package Foswiki::Configure::FeedbackCheckers::LOGVIEWER

Foswiki::Configure::FeedbackCheckers::LOGVIEWER
...
sub provideFeedback {
    ...
    my $startTime = $Foswiki::Configure::query->param('{ConfigureGUI}{LogViewer}{StartTime}');
    $startTime = Foswiki::Time::parseTime($startTime) if ( defined $startTime );

    my $endTime = $Foswiki::Configure::query->param('{ConfigureGUI}{LogViewer}{EndTime}');
...
sub plainViewer {
    my ( $this, $keys, $startTime, $endTime ) = @_;

    my $status  = '';
    my $content = '';
    my $dir     = $this->{item}{dir};
    my $file    = $Foswiki::Configure::query->param($keys);
...
sub displayConfigureLog {
    my ( $this, $log, $startTime, $endTime ) = @_;
...
"<table><tbody><tr class='configureLogHeader'><td>Seq<td>Date<td>Severity<td colspan='99'>Event Detail";
    ...
    my $severities = [ $Foswiki::Configure::query->param($keys) ];
