...
around BUILDARGS => sub {
    my $orig = shift;
    return $orig->( @_, testSuite => 'URLPARAM' );
};
...
sub test_default {
    ...
    $str =
      $this->test_topicObject->expandMacros('%URLPARAM{"foo" default="bar"}%');
...
sub test_encode {
    ...
    $this->request->param( -name => 'foo', -value => '&?*!" ' );
    $str =
      $this->test_topicObject->expandMacros('%URLPARAM{"foo" encode="quote"}%');
    $this->assert_str_equals( '&?*!\" ', $str );

    $this->request->param( -name => 'foo', -value => '<evil script>\'\"%' );
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="bar" encode="safe"}%');
    $this->assert_str_equals( '&#60;evil script&#62;&#39;\&#34;&#37;', $str );

    $this->request->param( -name => 'foo', -value => '<evil script>\'\"%' );
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="bar" encode="off"}%');
    $this->assert_str_equals( '<evil script>\'\"%', $str );

    $this->request->param( -name => 'foo', -value => '<evil script>\'\"%' );
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="bar" encode="none"}%');
    $this->assert_str_equals( '<evil script>\'\"%', $str );

    $this->request->param( -name => 'foo', -value => '<evil "script">\'\"%' );
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="bar" encode="quote, safe"}%');
    ...
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="bar" encode="safe, quote"}%');
    $this->assert_str_equals(
        '&#60;evil &#34;script&#34;&#62;&#39;\\&#34;&#37;', $str );
}
...
sub test_defaultencode {
    ...
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="&?*!\" " encode="entity"}%');
    $this->assert_str_equals( '&?*!" ', $str );

    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="&?*!\" " encode="url"}%');
    $this->assert_str_equals( '&?*!" ', $str );

    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"foo" default="&?*!\" " encode="quote"}%');
    $this->assert_str_equals( '&?*!" ', $str );
}
...
sub test_multiple {
    ...
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"multi" multiple="on" separator=","}%');
    ...
    $str =
      $this->test_topicObject->expandMacros(
        '%URLPARAM{"multi" multiple="-$item-" separator=" "}%');
    $this->assert_str_equals( "-foo- -bar- -baz-", $str );
