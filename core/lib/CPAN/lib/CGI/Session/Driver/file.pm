...
"retrieve(): '$path' appears to be a symlink and I couldn't remove it: $!"
    ...
    $self->{NoFlock} || flock( FH, LOCK_SH )
      or return $self->set_error("retrieve(): couldn't lock '$path': $!");
...
"store(): '$path' appears to be a symlink and I couldn't remove it: $!"
    ...
    sysopen( FH, $path, $mode, $self->{UMask} )
      or return $self->set_error("store(): couldn't open '$path': $!");

    # sanity check to make certain we're still ok
    if ( -l $path ) {
        return $self->set_error(
            "store(): '$path' is a symlink, check for malicious processes");
    }

    # prevent race condition (RT#17949)
    $self->{NoFlock} || flock( FH, LOCK_EX )
      or return $self->set_error("store(): couldn't lock '$path': $!");
    truncate( FH, 0 )
      or return $self->set_error("store(): couldn't truncate '$path': $!");
...
sub remove {
    ...
    my $path  = $self->_file($sid);
    unlink($path)
      or return $self->set_error("remove(): couldn't unlink '$path': $!");
    return 1;
}
