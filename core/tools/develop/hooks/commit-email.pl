...
#
# $HeadURL: https://svn.collab.net/repos/svn/branches/1.0.x/hook-scripts/commit-email.pl.in $
...
my @dirschanged =
  &read_from_process( $svnlook, 'dirs-changed', $repos, '-r', $rev );
...
my @difflines =
  &read_from_process( $svnlook, 'diff', $repos, '-r', $rev, @no_diff_deleted );
