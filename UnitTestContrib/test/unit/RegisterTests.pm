...
sub set_up {
    ...
    try {
        $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
            $this->{users_web}, 'NewUserTemplate', <<'EOF');
%NOP{Ignore this}%
But not this
%SPLIT%
\t* Set %KEY% = %VALUE%
...
EOF
        ...
        $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
            $this->{users_web}, $Foswiki::cfg{SuperAdminGroup}, <<EOF);
   * Set GROUP = $this->{test_user_wikiname}
EOF

        $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
            $this->{users_web}, 'UserForm', <<'EOF');
...
sub verify_userTopicWithoutPMWithForm {
    ...
    my $m =
      new Foswiki::Meta( $this->{twiki}, $this->{users_web},
        'NewUserTemplate' );
    ...
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{users_web}, 'NewUserTemplate', <<'EOF', $m );
%SPLIT%
\t* Set %KEY% = %VALUE%
%SPLIT%
EOF
...
sub verify_shortPassword {
    ...
    try {
        ...
        $this->assert( $this->{twiki}->{users}->userExists($cUID),
            "new user created" );
    }
    catch Foswiki::OopsException with {
        my $e = shift;
...
sub verify_resetPasswordOkay {
    ...
    $this->registerAccount();
    my $cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $this->{new_user_login} );
    $this->assert( $this->{twiki}->{users}->userExists($cUID),
        " $cUID does not exist?" );
    my $newPassU = '12345';
    ...
    $this->assert(
        !$this->{twiki}->{users}->checkPassword( $cUID, $newPassU ) );
...
sub verify_buildRegistrationEmail {
    ...
    my $actual =
      Foswiki::UI::Register::_buildConfirmationEmail( $this->{twiki}, \%data,
