package TopicACLReadOnlyAccessControlTests;
use strict;
use warnings;
...
#can't change to ReadOnlyAccess  here, as we need to be able to create topics.
#$Foswiki::cfg{AccessControl} = 'Foswiki::Access::TopicACLReadOnlyAccess'
...
THIS
    ...
    $this->PERMITTED( "VIEW", $MrGreen );    # Due to duplicate DENYTOPICVIEW
    $this->DENIED( "VIEW",   $MrYellow );
    $this->DENIED( "VIEW",   $MrOrange );
    $this->DENIED( "VIEW",   $MrWhite );     # Due to ResevoirDogs
    $this->DENIED( "VIEW",   $MrBlue );
    $this->DENIED( "CHANGE", $MrBlue );
    $this->PERMITTED( "VIEW", 'BaseUserMapping_333' );

    return;
}

# Test that extended action ACLs are denied
#   - Extended ACL's are presumed non-Read-only
sub test_denytopic_extended {
    my $this = shift;
    my ($topicObject) =
      Foswiki::Func::readTopic( $this->{test_web}, $this->{test_topic} );
    $topicObject->text(<<"THIS");
...
THIS
    ...
    $this->DENIED( "WIBBLE", $MrGreen );
    $this->DENIED( "WIBBLE", $MrYellow );
    $this->DENIED( "WIBBLE", $MrOrange );
    $this->DENIED( "WIBBLE", $MrWhite );    # Due to ResevoirDogs
    $this->DENIED( "WIBBLE", $MrBlue );
    $this->PERMITTED( "WIBBLE", 'BaseUserMapping_333' );
...
THIS
    ...
    $this->DENIED( "CHANGE", $MrGreen );
    $this->DENIED( "CHANGE", $MrYellow );
    $this->DENIED( "CHANGE", $MrOrange );
    $this->DENIED( "CHANGE", $MrWhite );
    $this->DENIED( "CHANGE", $MrBlue );
...
If ALLOWTOPIC is set
   ...
   * Set ALLOWTOPICCHANGE = %USERSWEB%.MrOrange
THIS
    $topicObject->save();
    $topicObject->finish();
...
If ALLOWWEB is set to a list of wikinames
   ...
   * Set ALLOWWEBCHANGE = MrGreen MrYellow MrWhite
THIS
    );
    $topicObject->save();
    $topicObject->finish();
...
If ALLOWTOPIC is set
   ...
   * Set ALLOWTOPICCHANGE = MrYellow,%USERSWEB%.MrOrange,Nosuchweb.MrGreen,%MAINWEB%.MrBlue,%SYSTEMWEB%.MrWhite
THIS
    );
    $topicObject->save();
    $topicObject->finish();
...
sub _checkSettings {
    ...
    $this->assert(
        !$meta->haveAccess( 'CHANGE', $MrOrange ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
    $this->assert(
        !$meta->haveAccess( 'CHANGE', $MrGreen ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
    $this->assert(
        !$meta->haveAccess( 'CHANGE', $MrYellow ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
    $this->assert(
        !$meta->haveAccess( 'CHANGE', $MrWhite ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
    $this->assert(
        !$meta->haveAccess( 'CHANGE', $MrBlue ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
    $this->assert(
        $meta->haveAccess( 'CHANGE', 'BaseUserMapping_333' ),
        " 'CHANGE' $this->{test_web}.$this->{test_topic}"
    );
...
THIS
    ...
    $topicObject->putKeyed( 'PREFERENCE', $args );
    $topicObject->putKeyed( 'PREFERENCE', $args2 );
    $topicObject->save();
    $topicObject->finish();
...
THIS
    ...
    $this->DENIED( "CHANGE", $MrOrange, $subweb );
    $this->DENIED( "CHANGE", $MrGreen,  $subweb );
    $this->DENIED( "CHANGE", $MrGreen );
    $this->DENIED( "CHANGE", $MrOrange );
    $this->PERMITTED( "CHANGE", 'BaseUserMapping_333' );
...
THIS
    ...
    $this->DENIED( "CHANGE", $MrGreen,  $subweb );    # Readonly
    $this->DENIED( "VIEW",   $MrOrange, $subweb );
    $this->PERMITTED( "VIEW", $MrGreen );
    $this->DENIED( "CHANGE", $MrGreen );              # Readonly
    $this->DENIED( "VIEW",   $MrOrange );
    $this->PERMITTED( "VIEW", 'BaseUserMapping_333' );
...
sub test_finalised_parent_overrides_subweb {
   ...
   * Set ALLOWWEBVIEW = MrGreen
   * Set ALLOWWEBCHANGE = MrGreen
   * Set FINALPREFERENCES = ALLOWWEBVIEW, ALLOWWEBCHANGE
THIS
    $topicObject->save();
    $topicObject->finish();
