;(function($){$.wikiword={build:function(source,options){$.log("called wikiword()");var opts=$.extend({},$.fn.wikiword.defaults,options);var $source=$(source);return this.each(function(){var $this=$(this);var thisOpts=$.meta?$.extend({},opts,$this.data()):opts;$source.change(function(){$.wikiword.handleChange($source,$this,thisOpts);}).keyup(function(){$.wikiword.handleChange($source,$this,thisOpts);}).change();});},handleChange:function(source,target,thisOpts){var result='';source.each(function(){result+=$(this).is(':input')?$(this).val():$(this).text();});if(result||!thisOpts.initial){result=$.wikiword.wikify(result);if(thisOpts.suffix){result+=thisOpts.suffix;}
if(thisOpts.prefix){result=thisOpts.prefix+result;}}else{result=thisOpts.initial;}
$.log("result="+result);target.each(function(){$(this).is(':input')?$(this).val(result):$(this).text(result);});},wikify:function(source){var result='';var chVal=0;for(var i=0;i<source.length;i++){chVal=source.charCodeAt(i);if(chVal==192||chVal==193||chVal==194||chVal==195){ch='A';}
