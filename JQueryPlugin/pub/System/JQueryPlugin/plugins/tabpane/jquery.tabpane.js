...
thisOpts.currentTabId=newTabId;}},autoMaxExpand:function($thisPane,opts){window.setTimeout(function(){jQuery.tabpane.fixHeight($thisPane,opts);jQuery(window).one("resize",function(){$.tabpane.autoMaxExpand($thisPane,opts)});},100);},fixHeight:function($thisPane,opts){jQuery.log("tabpane: called fixHeight()");var $container=$thisPane.find("> .jqTab.current .jqTabContents");var paneOffset=$container.offset();if(typeof(paneOffset)=='undefined'){return;}
...
jQuery.log("tabpane: container="+$container.parent().attr('id')+" paneTop="+paneTop+" bottomBarHeight="+bottomBarHeight+" height="+height+" minHeight="+opts.minHeight);if(opts&&opts.minHeight&&height<opts.minHeight){jQuery.log("tabpane: minHeight reached");height=opts.minHeight;}
if(height<0){return;}
