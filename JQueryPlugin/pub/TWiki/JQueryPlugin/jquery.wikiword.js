$.wikiword={build:function(source,options){$.wikiword.writeDebug("called wikiword()");var opts=$.extend({},$.fn.wikiword.defaults,options);var $source=$(source);return this.each(function(){var $this=$(this);var thisOpts=$.meta?$.extend({},opts,$this.data()):opts;$source.change(function(){$.wikiword.handleChange($source,$this);}).keyup(function(){$.wikiword.handleChange($source,$this);}).change();});},handleChange:function(source,target){var result='';source.each(function(){result+=$(this).is(':input')?$(this).val():$(this).text();});result=$.wikiword.wikify(result);if(thisOpts.suffix){result+=thisOpts.suffix;}
if(thisOpts.prefix){result=prefix+result;}
...
result=result.replace(/[a-zA-Z\d]+/g,function(a){return a.charAt(0).toLocaleUpperCase()+a.substr(1);});result=result.replace(/[^a-zA-Z\d]/g,"");result=result.replace(/\s/g,"");return result;},writeDebug:function(msg){if($.wikiword.defaults.debug){if(window.console&&window.console.log){window.console.log("DEBUG: wikiword - "+msg);}else{}}},defaults:{debug:true,suffix:'',prefix:''}};$(function(){$.fn.wikiword=$.wikiword.build;});;
