...
sub _addRevision {
    ...
    if ( $this->{state} eq 'nocommav' && -e $this->{file} ) {
        ...
        $this->{head} = 1;
        $this->{revs}[1]->{text} =
          Foswiki::Store::RcsFile::readFile( $this, $this->{file} );
        $this->{revs}[1]->{log}    = $log;
    ...
    if ($isStream) {
        Foswiki::Store::RcsFile::saveStream( $this, $data );

        # SMELL: for big attachments, this is a dog
        $data = Foswiki::Store::RcsFile::readFile( $this, $this->{file} );
    }
    else {
        Foswiki::Store::RcsFile::saveFile( $this, $this->{file}, $data );
    }
...
sub _delLastRevision {
    ...
    Foswiki::Store::RcsFile::saveFile( $this, $this->{file}, $lastText );
}
