...
#print STDERR "---- create new $web . $topic\n";
    ...
    if (not defined($this->{cache}->{$web}{$topic}->{$this->{session}->{user}})) {
        $this->{cache}->{$web}{$topic}->{$this->{session}->{user}} = ();
    }
    if (not defined($this->{cache}->{$web}{$topic}->{$this->{session}->{user}}{allowView})) {
        $this->{cache}->{$web}{$topic}->{$this->{session}->{user}}{allowView} =
            $this->{cache}->{$web}{$topic}->{tom}->haveAccess('VIEW');
    }
    #use the cached permission
    $this->{cache}->{$web}{$topic}->{allowView} = $this->{cache}->{$web}{$topic}->{$this->{session}->{user}}{allowView};
