...
::_loadBasicModule(
    qw {
      ...
      Foswiki::Configure::TypeUI
      Foswiki::Configure::TypeUIs::BOOLEAN
      Foswiki::Configure::TypeUIs::NUMBER
      Foswiki::Configure::TypeUIs::SELECT
      Foswiki::Configure::TypeUIs::STRING
      Foswiki::Configure::LoadSpec
      Foswiki::Configure::SaveLSC
...
sub unsavedChangesNotice {
    ...
    # Remove any {ConfigureGUI} pseudo-keys from %updated
    foreach my $keys ( keys %$updated ) {
        delete $updated->{$keys} if $keys =~ /^{ConfigureGUI}/;
    }
    $template->addArgs( pendingCount => scalar keys %$updated );
