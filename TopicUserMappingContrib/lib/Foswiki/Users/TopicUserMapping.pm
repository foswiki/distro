...
around BUILDARGS => sub {
    ...
    my $params = $orig->( $class, @_, mapping_id => $FOSWIKI_USER_MAPPING_ID );
...
around handlesUser => sub {
    ...
    if ( defined $cUID && !length($mapping_id) ) {
        ...
        return 1 if ( defined $cUID && $cUID =~ m/^($mapping_id)/ );
    }
...
sub getLoginName {
    ...
    $login =~ s/$mapping_id// if $this->mapping_id;
...
if you fail to create a new user (for eg your Mapper has read only access),
            Foswiki::Exception->throw(
               text => 'Failed to add user: '.$ph->error());
...
'User exists in the Password Manager,  and the password you provided is different from the users current password.   You cannot add a user and change the password at the same time.'
        ...
        unless ( $this->passwords->setPassword( $login, $password ) == 1 ) {
            ...
            Foswiki::Exception->throw(
                text => $this->app->i18n->maketext('Failed to add user: ')
...
sub _maintainUsersTopic {
      ...
      )
    {
        ...
        my $templateTopicObject =
          Foswiki::Meta->load( $this->app, $cfgData->{SystemWebName},
            'UsersTemplate' );
        $usersTopicObject = $this->create(
            'Foswiki::Meta',
            web   => $cfgData->{UsersWebName},
    ...
    my $today =
      Foswiki::Time::formatTime( time(), $cfgData->{DefaultDateFormat},
        'gmtime' );
...
by $this->app->users->getCanonicalUserID($identity).
...
sub removeUser {
   ...
   # Then the WikiUsers topic will not be maintained.
    ...
    $this->clear_U2W;
    $this->clear_W2U;
    $this->_clear_eachGroupMember;
    $this->clear_singleGroupMembers;
...
sub getWikiName {
    ...
    my $cfgData    = $this->app->cfg->data;
    ASSERT($cUID) if DEBUG;
    ASSERT( $cUID =~ m/^$mapping_id/ ) if DEBUG;
    ...
    unless ($wikiname) {
        ...
        if ($wikiname) {
            ...
            $wikiname =~ s/$cfgData->{NameFilter}//g;
        }
    }
...
sub userExists {
    ...
    return 1 if $cUID eq $this->app->user;

    my $loginName = $this->getLoginName($cUID);
    return 0 unless defined($loginName);

    return 1 if ( $loginName eq $cfgData->{DefaultUserLogin} );
    ...
    unless ( $cfgData->{Register}{AllowLoginName}
        && $this->passwords->canFetchUsers() )
    {
        ...
        my $wikiname = $this->app->users->getWikiName($cUID);
        if ( Foswiki::Func::topicExists( $cfgData->{UsersWebName}, $wikiname ) )
        {
            return 1;
        }
    }
...
sub eachUser {
    ...
    my $iter = new Foswiki::ListIterator( list => \@list );
    $iter->{filter} = sub {

        # don't claim users that are handled by the basemapping
        my $cUID     = $_[0] || '';
        my $login    = $this->app->users->getLoginName($cUID);
        my $wikiname = $this->app->users->getWikiName($cUID);

        return !(
            $this->app->users->basemapping->handlesUser(
                undef, $login, $wikiname
            )
        );
    };
    return $iter;
...
sub eachGroupMember {
    ...
    if ( !$expand && defined( $this->singleGroupMembers->{$group} ) ) {
        ...
        return new Foswiki::ListIterator(
            list => $this->singleGroupMembers->{$group} );
    }

    if ( $expand && defined( $this->_eachGroupMember->{$group} ) ) {

        #        print STDERR "Returning cached expanded list for $group\n";
        return new Foswiki::ListIterator(
            list => $this->_eachGroupMember->{$group} );
    }
...
# Determine if we are called recursively, either directly, or by the _expandUserList routine
    ...
    if (  !$expanding{$group}
        && $app->store->topicExists( $cfgData->{UsersWebName}, $group ) )
    {
        $expanding{$group} = 1;

        #        print "Expanding $group \n";
        my $groupTopicObject =
          Foswiki::Meta->load( $this->app, $cfgData->{UsersWebName}, $group );
...
#            print "Returning iterator for singleGroupMembers $group, members $singleGroupMembers \n";
            ...
            $this->_eachGroupMember->{$group} = $members;
        }
...
sub isGroup {
    ...
    return 1 if $user eq $cfgData->{SuperAdminGroup};
...
sub eachMembership {
    ...
    my $it = new Foswiki::ListIterator( list => \@{ $this->groupsList } );
    $it->{filter} = sub {
...
sub groupAllowsView {
    ...
    return 1 if $this->app->users->isAdmin($user);
...
sub addUserToGroup {
    ...
    Foswiki::Exception->throw(
        textt => $this->app->i18n->maketext('Group names must end in Group') )
    ...
    if ( $usersObj->isGroup($groupName) ) {
        ...
        $groupTopicObject =
          Foswiki::Meta->load( $this->app, $groupWeb, $groupName );

        if ( !$groupTopicObject->haveAccess( 'CHANGE', $user ) ) {
            Foswiki::Exception->throw(
                text => $this->app->i18n->maketext(
                    'CHANGE not permitted by [_1]', $user
                )
            );
        }
        ...
        foreach my $ident ( split( /[\,\s]+/, $membersString ) ) {
            $ident =~ s/^($cfgData->{UsersWebName}|%USERSWEB%|%MAINWEB%)\.//;
...
# see if we have permission to add a topic, or to edit the existing topic, etc..
        ...
        $groupTopicObject =
          Foswiki::Meta->load( $this->app, $groupWeb, 'GroupTemplate' );
    ...
    $this->_getListOfGroups(1) if ($create);
    return 1;
}
...
#TODO: consider removing the VIEW_TEMPLATE that only very few people should ever have...
    ...
    $groupTopicObject->saveAs(
        ...
        author => $user,
        forcenewrevision =>
          ( $groupName eq $this->app->cfg->data->{SuperAdminGroup} )
        ? 1
        : 0
    );
...
sub removeUserFromGroup {
    ...
    Foswiki::Exception->throw(
        text => $this->app->i18n->maketext(
            'Users cannot be removed from [_1]', $groupName
        )
    ...
    Foswiki::Exception->throw(
        text => $this->app->i18n->maketext(
            ...
            ( $cfgData->{AdminUserWikiName}, $cfgData->{SuperAdminGroup} )
        )
      )
      if ( $groupName eq $cfgData->{SuperAdminGroup}
    ...
    if (
        ...
        and (
            $this->app->store->topicExists(
                $cfgData->{UsersWebName}, $groupName
            )
        )
      )
    {
...
sub _clearGroupCache {
    ...
    delete $this->_eachGroupMember->{$groupName};
    ...
    foreach my $groupKey ( keys( %{ $this->singleGroupMembers } ) ) {
        if ( $this->singleGroupMembers->{$groupKey} =~ m/$groupName/ ) {
            ...
            delete $this->_eachGroupMember->{$groupKey};
...
sub isAdmin {
    ...
    if ( $cUID eq $this->app->cfg->data->{SuperAdminGroup} ) {
        $isAdmin = 1;
    }
    else {
        my $sag = $this->app->cfg->data->{SuperAdminGroup};
...
sub findUserByEmail {
    ...
    my @users;
    if (  !$this->app->cfg->data->{TopicUserMapping}{ForceManageEmails}
    ...
    if ( $seen->{$user} ) {
        ...
        if ( $this->isGroup($user) ) {
                ...
                && $this->passwords->isManagingEmails() )
            {
                ...
                foreach ( mapper_getEmails( $this->app, $user ) ) {
                    $emails{$_} = 1;
                }
            }
        }
...
sub setEmails {
        ...
        && $this->passwords->isManagingEmails() )
    {
        ...
        mapper_setEmails( $this->app, $user, @_ );
    }
}

=begin TML

---++ StaticMethod mapper_getEmails($app, $user)
...
---++ StaticMethod mapper_setEmails ($app, $user, @emails)
...
sub mapper_setEmails {
    ...
    my $user = $app->users->getWikiName($cUID);
...
sub findUserByWikiName {
    ...
    elsif ( $this->app->cfg->data->{Register}{AllowLoginName} ) {
...
sub checkPassword {
 ...
 # If we don't have a PasswordManager and use TemplateLogin,  always allow login
    return 1
      if ( $this->app->cfg->data->{PasswordManager} eq 'none'
        && $this->app->cfg->data->{LoginManager} eq
...
Returns a string containing the sanitized registration field, or can throw an Foswiki::Exception
...
sub _expandUserList {
    ...
    foreach my $ident ( split( /[\,\s]+/, $names ) ) {
        ...
        if ( $this->isGroup($ident) ) {
            ...
            my %namelist =
              map { $_ => 1 }
              @{ $this->app->users->findUserByWikiName($ident) };
            ...
            unless (%namelist) {
                my $cUID = $this->app->users->getCanonicalUserID($ident)
                  || $ident;
