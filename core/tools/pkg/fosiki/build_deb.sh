...
if [ ! -e Foswiki-1.1.6.tgz ]; then
	echo 'need Foswiki-1.1.6.tgz file to build'
	exit -1;
fi

mkdir /tmp/build_deb
cp -r debian /tmp/build_deb/
cp Foswiki-1.1.6.tgz /tmp/build_deb/foswiki_1.1.6.orig.tar.gz

cd /tmp/build_deb
tar zxvf /tmp/build_deb/foswiki_1.1.6.orig.tar.gz

#add * to allow for -beta, -auto etc
mv /tmp/build_deb/Foswiki-1.1.6*/ /tmp/build_deb/foswiki-1.1.6/
cd /tmp/build_deb/foswiki-1.1.6
