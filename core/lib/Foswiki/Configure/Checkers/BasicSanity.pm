...
sub checkSanity {
    ...
    $Foswiki::Configure::defaultCfg = _copy( \%Foswiki::cfg );

    if ( !$this->{LocalSiteDotCfg} ) {
        $this->{errorcount}++;
        $result .= <<HERE;
Could not find where =LocalSite.cfg= is supposed to go.
<p>Edit =LocalLib.cfg= and set =\$twikiLibPath= to point to the 'lib' directory of your installation.
...
HERE
        ...
        if ($@) {
            $this->{errorcount}++;
            $result .= <<HERE;
Existing configuration file =$this->{LocalSiteDotCfg}= has a problem
...
<p>Manually edit and correct =$this->{LocalSiteDotCfg}= file if you
...
HERE
            ...
            $this->{errorcount}++;
            $result .= <<HERE;
Cannot write to existing configuration file: =$this->{LocalSiteDotCfg}= is not writable. <br />
...
HERE
            ...
            $this->{errorcount}++;
            $result .= <<HERE;
The existing configuration file =$this->{LocalSiteDotCfg}= doesn't seem to contain a good configuration for Foswiki. The following problems were found:<br />
$mess
...
HERE
        ...
        if ($errs) {
            $this->{errorcount}++;
            $result .= <<HERE;
Configuration file =$this->{LocalSiteDotCfg}= does not exist, and I cannot write a new configuration file due to these errors:
<pre/>$errs<pre>
