...
sub renderParent {
    ...
    while ($parent) {
        ...
        $parent = $pWeb . '.' . $pTopic;
        last
          if ( $noWebHome && ( $pTopic eq $TWiki::cfg{HomeTopicName} )
            || $visited{$parent} );
        $visited{$parent} = 1;
        $text = $format;
        $text =~ s/\$web/$pWeb/g;
...
sub _emitTR {
    ...
    $theRow =~ s/(\|\|+)/'colspan'.$TWiki::TranslationToken.length($1).'|'/ge
      ;                       # calc COLSPAN
    my $cells = '';
...
sub _handleSquareBracketedLink {
    ...
    if ( $link =~ m!^($TWiki::regex{linkProtocolPattern}\:|/)! ) {
        if ( defined $text ) {
            ...
            $text =~
s/(?<=[\s\(])($TWiki::regex{wikiWordRegex}|[$TWiki::regex{upperAlpha}])/<nop>$1/go;
        }
        else {
...
sub _externalLink {
    ...
    if ( $url =~ /^mailto:/i ) {
        if ( $TWiki::cfg{AntiSpam}{EmailPadding} ) {
            $url =~
              s/(\@[\w\_\-\+]+)(\.)/$1$TWiki::cfg{AntiSpam}{EmailPadding}$2/;
            if ($text) {
                $text =~
s/(\@[\w\_\-\+]+)(\.)/$1$TWiki::cfg{AntiSpam}{EmailPadding}$2/;
            }
...
sub getRenderedVersion {
    ...
    $text =~
s/<(\/?\w+(:\w+)?)>/{$TWiki::TranslationToken$1}$TWiki::TranslationToken/g;
    $text =~
s/<(\w+(:\w+)?(\s+.*?|\/)?)>/{$TWiki::TranslationToken$1}$TWiki::TranslationToken/g;

    # XML processing instruction only valid at start of text
    $text =~
      s/^<(\?\w.*?\?)>/{$TWiki::TranslationToken$1}$TWiki::TranslationToken/g;
...
# SMELL: in case this type of anchor (presumably user-defined) gets renamed, it should be noted somewhere
    $text =~
s/^(\#)($TWiki::regex{wikiWordRegex})/CGI::a({name=>$this->makeUniqueAnchorName($theWeb,$theTopic,$2)},'')/geom;

    # Headings
    # '<h6>...</h6>' HTML rule
    $text =~
s/$TWiki::regex{headerPatternHt}/_makeAnchorHeading($this,$2,$1,$theTopic,$theWeb)/geo;

    # '----+++++++' rule
    $text =~
s/$TWiki::regex{headerPatternDa}/_makeAnchorHeading($this,$2,(length($1)),$theTopic,$theWeb)/geo;
...
s/\[\[([^\]\[\n]+)\](\[([^\]\n]+)\])?\]/_handleSquareBracketedLink( $this,$theWeb,$theTopic,$1,$3)/ge;
    ...
    unless ( TWiki::isTrue( $prefs->getPreferencesValue('NOAUTOLINK') ) ) {
        ...
        $text =~
s/$STARTWW(?:($TWiki::regex{webNameRegex})\.)?($TWiki::regex{wikiWordRegex}|$TWiki::regex{abbrevRegex})($TWiki::regex{anchorRegex})?/_handleWikiWord( $this,$theWeb,$1,$2,$3)/geom;
...
s/$STARTWW((mailto\:)?[a-zA-Z0-9-_.+]+@[a-zA-Z0-9-_.]+\.[a-zA-Z0-9-_]+)$ENDWW/_mailLink( $this, $1 )/gem;
    ...
    $text =~
s/\[\[$TWiki::regex{linkProtocolPattern}\:([^\s<>"]+[^\s*.,!?;:)<|])\s+(.*?)\]\]/$3/g;
    $text =~ s/\[\[([^\]]*\]\[)(.*?)\]\]/$2/g;

    # remove "Web." prefix from "Web.TopicName" link
    $text =~
s/$STARTWW(($TWiki::regex{webNameRegex})\.($TWiki::regex{wikiWordRegex}|$TWiki::regex{abbrevRegex}))/$3/g;
...
#    $text =~ s/(^|(<=\W))((($TWiki::regex{webNameRegex})\.)?($TWiki::regex{wikiWordRegex}|$TWiki::regex{abbrevRegex}))/<nop>$1/g;
    $text =~
s/((($TWiki::regex{webNameRegex})\.)?($TWiki::regex{wikiWordRegex}|$TWiki::regex{abbrevRegex}))/<nop>$1/g;
...
sub makeTopicSummary {
    ...
    $htext =~
      s/^(.{$nchar}.*?)($TWiki::regex{mixedAlphaNumRegex}).*$/$1$2 \.\.\./s;
...
sub _putBackProtected {
    ...
    foreach my $placeholder ( keys %$map ) {
        ...
        $$text =~
s/<!--$TWiki::TranslationToken$placeholder$TWiki::TranslationToken-->/$val/;
...
s(<!--$TWiki::TranslationToken$placeholder$TWiki::TranslationToken-->)($val);
                ...
                $$text =~
s(<!--$TWiki::TranslationToken$placeholder$TWiki::TranslationToken-->)
...
sub renderRevisionInfo {
    ...
    $value =~
s/\$date/TWiki::Time::formatTime( $date, $TWiki::cfg{DefaultDateFormat} )/gei;
    $value =~
      s/(\$(rcs|http|email|iso))/TWiki::Time::formatTime($date, $1 )/gei;
