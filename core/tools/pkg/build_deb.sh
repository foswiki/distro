...
if [ ! -e Foswiki-1.0.5.tgz ]; then
	echo 'need Foswiki-1.0.5.tgz file to build'
	exit -1;
fi

mkdir /tmp/build_deb
cp -r debian /tmp/build_deb/
cp Foswiki-1.0.5.tgz /tmp/build_deb/foswiki_1.0.5.orig.tar.gz

cd /tmp/build_deb
tar zxvf /tmp/build_deb/foswiki_1.0.5.orig.tar.gz

#add * to allow for -beta, -auto etc
mv /tmp/build_deb/Foswiki-1.0.5*/ /tmp/build_deb/foswiki-1.0.5/
cd /tmp/build_deb/foswiki-1.0.5
