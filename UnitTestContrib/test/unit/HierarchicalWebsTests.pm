...
sub test_WEBLIST_all {
    ...
    my $topicObject =
      Foswiki::Meta->new( $this->{session}, $testWeb, 'WebHome' );
    $text = $topicObject->expandMacros($text);
    foreach my $web (
        'HierarchicalWebsTestsTestWeb',
        'HierarchicalWebsTestsTestWeb/SubWeb',
        'Main', 'Sandbox', 'System', 'TestCases'
      )
    {
...
sub test_WEBLIST_relative {
    ...
    my $text =
      ' %WEBLIST{format="$name" separator=", " subwebs="' . $testWeb . '"}% ';
    my $topicObject =
      Foswiki::Meta->new( $this->{session}, $testWeb, 'WebHome' );
    $text = $topicObject->expandMacros($text);
