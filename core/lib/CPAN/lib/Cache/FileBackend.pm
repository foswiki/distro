...
sub delete_namespace {
    ...
    _Recursively_Remove_Directory(
        Build_Path( $self->get_root(), $p_namespace ) );
}
...
sub _Create_Directory {
    ...
    -d $directory
      or throw Error::Simple("Couldn't create directory: $directory: $!");
...
sub _Read_Dirents {
    ...
    opendir( Dir, _Untaint_Path($p_directory) )
      or throw Error::Simple("Couldn't open directory $p_directory: $!");

    my @dirents = readdir(Dir);

    closedir(Dir)
      or throw Error::Simple("Couldn't close directory $p_directory: $!");
...
sub _Read_File {
    ...
    open( File, _Untaint_Path($p_path) )
      or return undef;
...
sub _Write_File {
    ...
    -e $temp_filename
      or throw Error::Simple("Temp file '$temp_filename' does not exist: $!");

    rename( $temp_filename, _Untaint_Path($p_path) )
      or throw Error::Simple("Couldn't rename $temp_filename to $p_path: $!");
...
sub _get_key_for_unique_key {
    ...
    return $self->_read_data(
        $self->_path_to_unique_key( $p_namespace, $p_unique_key ) )->[0];
}
...
sub _path_to_unique_key {
    ...
    return Build_Path( $self->get_root(), $p_namespace,
...
sub _read_data {
    ...
    my $frozen_data_ref = _Read_File_Without_Time_Modification($p_path)
