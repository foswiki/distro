...
sub _longestCommonSubsequence {
    ...
    if ( ref($a) eq 'HASH' ) {    # prepared hash must be in $b
        my $tmp = $b;
        $b = $a;
        $a = $tmp;
    }
...
sub Diff {
    ...
    for my $seq ( 1, 2 ) {
        $ret |= $seq
          if $me->[_Idx][ $off + $seq + _Min ] < $me->[_Idx][ $off + $seq ];
    }
    return $ret;
}
...
sub Range {
    ...
    my $off = $me->_ChkSeq($seq);
    if ( !wantarray ) {
        return $me->[_Idx][$off] - $me->[_Idx][ $off + _Min ];
    }
    $base = $me->[_Base] if !defined $base;
    return ( $base + $me->[_Idx][ $off + _Min ] )
      .. ( $base + $me->[_Idx][$off] - 1 );
...
sub Items {
    ...
    return @{ $me->[$seq] }
      [ $me->[_Idx][ $off + _Min ] .. ( $me->[_Idx][$off] - 1 ) ];
}
