...
HERE
    ...
    if ($@) {
        $mess = $@;
        $mess =
            $this->ERROR('TWiki.pm could not be loaded. The error was:')
          . CGI::pre($mess)
          . $this->ERROR(<<HERE);
...
HERE
        ...
        $mess =
          'TWiki.pm (Version: <strong>' . $TWiki::VERSION . '</strong>) found';
    }
    ...
    $block .= $this->setting(
        ...
        $TWiki::query->path_info() . $this->NOTE(
            <<HERE
...
HERE
    ...
    $n .= $this->NOTE(
        'mod_perl is ',
        $TWiki::cfg{DETECTED}{ModPerlLoaded} ? '' : 'not',
        ' loaded into Apache'
    );
    ...
    if (   $TWiki::cfg{DETECTED}{UsingModPerl}
        && $TWiki::cfg{DETECTED}{ModPerlVersion} =~ /1\.99_?11/ )
    {
...
HERE
    ...
    $block .= $this->setting( 'Original PATH',
        $TWiki::cfg{DETECTED}{originalPath} . $this->NOTE(<<HERE) );
