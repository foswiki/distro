...
sub Build_Object {
    ...
    $object->set_expires_at(
        Build_Expires_At( $now, $p_default_expires_in, $p_expires_in ) );
    return $object;
}
...
sub get_object {
    ...
    my $object =
      $self->_get_backend()->restore( $self->get_namespace(), $p_key )
      or return undef;

    $object->set_size(
        $self->_get_backend()->get_size( $self->get_namespace(), $p_key ) );
...
sub set {
    ...
    $self->set_object(
        $p_key,
        Build_Object(
            $p_key, $p_data, $self->get_default_expires_in(),
            $p_expires_in
        )
    );
}
...
sub size {
    ...
    foreach my $key ( $self->get_keys() ) {
        $size +=
          $self->_get_backend()->get_size( $self->get_namespace(), $key );
    }
...
sub get_namespaces {
    ...
    return
      grep { !/$AUTO_PURGE_NAMESPACE/ } $self->_get_backend()->get_namespaces();
}
...
sub _insert_auto_purge_object {
    ...
    my $object =
      Build_Object( $p_real_namespace, 1, $self->get_auto_purge_interval(),
        undef );
