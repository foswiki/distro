...
#---+ Three
# ---++++ Three.1.1.One
# **SELECTCLASS Foswiki::Configure::TypeUIs::***
$cfg{Types}{Chosen} = 'Foswiki::Configure::TypeUIs::BOOLEAN';
1;
EXAMPLE
    $fh->close();
    do $fhname;
    ...
    $cfg{Types}{Chosen}    = 'Foswiki::Configure::TypeUIs::STRING';
...
\$Foswiki::cfg{Types}{Chosen} = 'Foswiki::Configure::TypeUIs::STRING';
1;
EXAMPLE
    my @a = split( "\n", $expectacle );
    my @b = split( "\n", $saver->{content} );
...
EXAMPLE
    ...
    my $controls = Foswiki::Configure::GlobalControls->new();
...
sub test_Package_loadInstaller {
    ...
    my $pkg =
      Foswiki::Configure::Package->new( $root, 'EmptyPlugin',
        $this->{session} );
...
sub test_Package_fetchFile {
    ...
    my ( $resp, $file );
    try {
        my $pkg = Foswiki::Configure::Package->new( $root, 'EmptyPlugin' );
        $pkg->repository($repository);
...
sub test_Package_errors {
    ...
    #
    my $pkg =
      Foswiki::Configure::Package->new( $root, 'EmptyPluginx', $this->{session},
