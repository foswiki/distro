...
sub init {
    ...
    unless ( -e $this->{file} ) {
        ...
        my ( $output, $exit ) =
          $TWiki::sandbox->sysCommand( 'svn add %FILENAME|F%',
            FILENAME => $this->{file} );
        if ($exit) {
            throw Error::Simple(
    ...
    for my $dir (@components) {
        if ( $dir =~ /\/+/ ) {
            ...
            if ( !-e "$path$dir" && -e "$path/.svn" ) {
                my ( $output, $exit ) =
                  TWiki::sandbox->sysCommand( 'svn mkdir %FILENAME|F%',
                    FILENAME => $path . $dir );
                if ($exit) {
...
sub moveTopic {
    ...
    # Move data file
    my $new =
      new TWiki::Store::Subversive( $this->{session}, $newWeb, $newTopic, '' );
...
sub copyTopic {
      ...
      )
    {
        for my $att ( grep { !/^\./ } readdir DIR ) {
            ...
            my $oldAtt =
              new TWiki::Store::Subversive( $this->{session}, $this->{web},
                $this->{topic}, $att );
...
sub moveAttachment {
    ...
    my $new =
      TWiki::Store::Subversive->new( $this->{session}, $newWeb, $newTopic,
        $newAttachment );
...
sub _rmtree {
    ...
    my ( $output, $exit ) =
      $TWiki::sandbox->sysCommand( 'svn rm %FILENAME|F%', FILENAME => $root );
    if ($exit) {
...
sub numRevisions {
    ...
    my ( $output, $exit ) =
      $TWiki::sandbox->sysCommand( 'svn info %FILE|F%', FILE => $this->{file} );
    if ($exit) {
