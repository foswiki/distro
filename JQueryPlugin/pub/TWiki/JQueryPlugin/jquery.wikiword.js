$.wikiword={build:function(source,options){$.wikiword.writeDebug("called wikiword()");var opts=$.extend({},$.fn.wikiword.defaults,options);var $source=$(source);return this.each(function(){var $this=$(this);var thisOpts=$.meta?$.extend({},opts,$this.data()):opts;$source.change(function(){$.wikiword.handleChange($source,$this);}).keyup(function(){$.wikiword.handleChange($source,$this);}).change();});},handleChange:function(source,target){var result='';source.each(function(){result+=$(this).is(':input')?$(this).val():$(this).text();});result=$.wikiword.wikify(result);target.each(function(){$(this).is(':input')?$(this).val(result):$(this).text(result);});},wikify:function(source){var result='';var chVal=0;for(var i=0;i<source.length;i++){chVal=source.charCodeAt(i);if(chVal==192||chVal==193||chVal==194||chVal==195){ch='A';}
...
result=result.capitalize();result=result.replace(/[^a-zA-Z\d]/g,"");result=result.replace(/\s/g,"");return result;},writeDebug:function(msg){if($.wikiword.defaults.debug){if(window.console&&window.console.log){window.console.log("DEBUG: wikiword - "+msg);}else{}}},defaults:{debug:false}};$(function(){$.fn.wikiword=$.wikiword.build;});;
