...
sub new {
    ...
    my $topicPubDir =
      $this->normalizeFileName( $this->{imagesDir} . "/$web/$topic" );
    ...
    if ( $this->{id} ) {
        ...
        $this->{infoFile} =
          $this->normalizeFileName("$this->{igpDir}/info.txt");
    }
...
#writeDebug("size=$this->{size} thumbsize=$thumbsize thumbwidth=$thumbwidth thumbheight=$thumbheight");
    ...
    foreach my $theTopic ( split( /,\s*/, $topics ) ) {
        my $theWeb;
        ( $theWeb, $theTopic ) =
          Foswiki::Func::normalizeWebTopicName( $this->{web}, $theTopic );
    ...
    unless ( defined $this->{frontend} ) {
        $this->{frontend} =
          ( Foswiki::Func::getContext()->{JQueryPluginEnabled} )
          ? 'lightbox'
          : 'default';
    }
...
sub render {
    ...
    $this->{infoChanged} = 1
      if !$this->{info}{thumbwidth}
          ...
          || join( ', ', $this->{info}{topics}{value} ) ne
          join( ', ', @{ $this->{topics} } );
    ...
    if ( $this->{frontend} eq 'lightbox' ) {
        ...
        $this->{header} =
"<noautolink><div class=\"$class {itemSelector:'.igpThumbNail', singleMode:true}\" id='igp$this->{id}'>\n";
        $this->{footer} =
          "<span class='foswikiClear'></span></div></noautolink>";
        $this->{format} =
"<a href='\$imageurl' class='igpThumbNail {origUrl:\"\$origurl\"}' style='width:"
          . $this->{thumbwidth}
          . "px; height:"
          . $this->{thumbheight}
          . "px;' title='\$comment'><img src='\$thumburl' alt='\$comment' /></a>"
...
sub renderImage {
    ...
    if ( $this->{doDocRels} ) {
        $result .=
          ...
          . $this->{session}
          ->getScriptUrl( 0, 'view', $this->{web}, $this->{topic} )
          . "' title='Thumbnails' />\n";
        if ( $firstImg && $firstImg->{name} ne $filename ) {
            $result .= "<link rel='first' href='"
              . $this->{session}->getScriptUrl(
                0, 'view', $this->{web}, $this->{topic},
                'id'       => $this->{id},
    ...
    # layout img table
    $result .=
"<table class='igpPicture' cellspacing='0' cellpadding='0'><tr><td colspan='2'>\n";
...
'<a href="$origurl"><img src="$imageurl" title="$comment" width="$width" height="$height"/></a>';
    ...
    if ( $firstImg && $firstImg->{name} ne $filename ) {
        ...
        $result .=
"<span class='igpNaviFirst igpNaviDisabled '><span>first</span></span>";
    }
    if ($prevImg) {
        ...
        $result .=
          "<span class='igpNaviPrev igpNaviDisabled '><span>prev</span></span>";
    }
    if ($nextImg) {
        ...
        $result .=
          "<span class='igpNaviNext igpNaviDisabled '><span>next</span></span>";
    }
    if ( $lastImg && $lastImg->{name} ne $filename ) {
        ...
        $result .=
          "<span class='igpNaviLast igpNaviDisabled '><span>last</span></span>";
    }
...
sub getImages {
    ...
    foreach my $webtopic ( @{ $this->{topics} } ) {
        ...
        my $viewAccessOK =
          Foswiki::Func::checkAccessPermission( "view", $this->{wikiName},
            undef, $theTopic, $theWeb );
        ...
        foreach my $image ( $meta->find('FILEATTACHMENT') ) {
            ...
            next
              if $this->{exclude}
                  && $image->{ $this->{field} } =~ /$this->{exclude}/;
            next
              if $this->{include}
                  && $image->{ $this->{field} } !~ /$this->{include}/;
       ...
       # SMELL work around for Image::Magick segfaulting reading svg image files
            ...
            $image->{IGP_filename} =
              $this->normalizeFileName( $this->{pubDir}
                  . "/$image->{IGP_web}/$image->{IGP_topic}/$image->{name}" );
            $image->{IGP_url} = $this->{pubUrlPath}
              . "/$image->{IGP_web}/$image->{IGP_topic}/$image->{name}";
...
sub replaceVars {
    ...
    if ($image) {
        ...
        $format =~
          s/\$username/Foswiki::Func::wikiToUserName($image->{user})/geos;
...
sub processImage {
    ...
    if ($thumbMode) {
        $error =
          $this->{mage}->Resize(
            geometry => "$image->{IGP_thumbwidth}x$image->{IGP_thumbheight}" )
          . " "
          . $this->{mage}->Crop(
            width  => $this->{thumbwidth},
            height => $this->{thumbheight}
        ...
        $error =
          $this->{mage}
          ->Resize( geometry => "$image->{IGP_width}x$image->{IGP_height}" );
    }
    if ( $error =~ /(\d+):/ ) {
...
sub readInfo {
    ...
    foreach my $line ( split( /\n/, $text ) ) {
        my $entry;
        if ( $line =~
/^name=(.*), version=(.*), origwidth=(.*), origheight=(.*), width=(.*), height=(.*), thumbwidth=(.*), thumbheight=(.*)$/
          )
        {
            $entry = {
                name        => $1,
