...
+src+"';");}
if((src=this._getImageSrc(onmouseout))!=""){src=eval(tinyMCE.settings['foswikipuburl_callback']
...
return;do{if(node.nodeName=="IMG"&&tinyMCE.getAttrib(node,'class').indexOf('mceItem')==-1){tinyMCE.switchClass(editor_id+'_foswikiimage','mceButtonSelected');return true;}}while((node=node.parentNode));tinyMCE.switchClass(editor_id+'_foswikiimage','mceButtonNormal');return true;},_getImageSrc:function(s){var sr,p=-1;if(!s)
return"";if((p=s.indexOf('this.src='))!=-1){sr=s.substring(p+10);sr=sr.substring(0,sr.indexOf('\''));return sr;}
return"";}};tinyMCE.addPlugin("foswikiimage",TinyMCE_FoswikiImagePlugin);