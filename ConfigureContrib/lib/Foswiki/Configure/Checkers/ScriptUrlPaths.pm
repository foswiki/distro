...
# Checker for entries in the {ScriptUrlPaths} hash
# Also used by the {ScriptUrlPath} checker
...
sub check {
    ...
    if ( $keys =~ /{ScriptUrlPaths}{([^}]+)}$/ ) {
        ...
        # undefined implies no alias
        if ( defined $value ) {
            ASSERT( $value, $script );
            $value ||= "BAD_URL_PATH '$Foswiki::cfg{ScriptUrlPaths}{$script}'";
        }
        else {
            $value =
    ...
    unless ($badLSC) {
        ...
        # the special parameter "configurationTest" kicks *all* foswiki
        # scripts into a special mode, where
        # Foswiki::Configure::ImageTest::respond generates a response.
        # therefore '$value' *must* end up as the valid name of a script
