...
foswiki.JQueryPlugin = new function () {
  var self = this;
}
...
$(function(){
  ...
  if (false) {
    $(".foswikiToc").each(function() { 
      $(this).prepend("<a class='foswikiTocToggle'>[hide]</a>")
    });

    /* behaviour of the foswiki toggle switch */
    $(".foswikiTocToggle").
      css("float","right").
      click(function() {
   ...
   */
  if (false) {
    $.fn.media.defaults.mp3Player = foswiki.pubUrlPath+'/'+foswiki.systemWebName+'/JQueryPlugin/mediaplayer/player.swf';
    $.fn.media.defaults.flvPlayer = foswiki.pubUrlPath+'/'+foswiki.systemWebName+'/JQueryPlugin/mediaplayer/player.swf';
