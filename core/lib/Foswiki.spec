...
# <p>Security Note:</b> Only the URL paths listed should
...
# <p>The tabs allow you to control most aspects of how Foswiki handles security
...
# **NUMBER 20 DISPLAY_IF="{UseClientSessions}"**
...
# **NUMBER CHECK="min:1" EXPERT DISPLAY_IF="{Validation}{Method}!='none'"**
...
# **NUMBER CHECK="min:10" EXPERT DISPLAY_IF="{Validation}{Method}!='none'"**
...
# are validated using this regex. (The user must still first login using the normal Foswiki authentication).
# If configured, it is applied as a replacement for testing the isAdmin status of the user.  This can be used to:
# <ul><li>Allow configure to be used only by a subset of Admins</li>
# <li>Allow configure to be used by non-admin users.</li>
# <li>Allow configure to run by anyone</li></ul>
# Because users with access to configure can install software on the server and make
# changes that are potentially difficult to recover from, it is strongly recommended that configure
# access be limited.   Examples:
# <ul><li>Restrict configure to "JoeAdmin" and "BobAdmin": <b><tt>^(JoeAdmin|BobAdmin)$</tt></li></b>
# <li>Restrict to the sudo admin user:  <b><tt>^(BaseUserMapping_333)$</tt></li></b>  (Also set the expert Password setting under the Passwords tab)
# <li>Completely unauthenticated access: <b><tt>.*</tt></b></ul>
...
# <dl>
# <dt>(HTTPS)</dt><dd> Any encoding over an HTTPS SSL connection. (Not a selection here.)</dd>
...
# **NUMBER CHECK="min:0" DISPLAY_IF="{PasswordManager}=='Foswiki::Users::HtPasswdUser' && {Htpasswd}{Encoding}=='bcrypt'"**
...
# "sudo" facility.  <b>As it is a "shared password", this is no longer recommended per good
# security practices and is not set by default.</b>  If you want to restore sudo access, set
# this field to a valid hashed password generated by the apache <tt>htpasswd</tt> command
# Ex: Set the sudo password to 'password'
# <pre>htpasswd -nb admin password
# admin:$apr1$3xBPRZAV$iqaC9QyWdzC/93os7A9np1 </pre>
# Paste the everything following the <tt>"admin:"</tt> into this field.
# Do not include the <tt>admin:</tt>
...
# <p>Certificates for Secure Email may be obtained from a vendor or private certificate authority.  You can also use the action buttons to generate certificates or certificate requests if OpenSSL is installed.
...
# provide your gmail email address and password for authentication, and click <strong>Auto-configure</strong>.</p>
...
# <p> Leave blank if you are using a Foswiki-generated Self-signed certificate
...
# <p> If you are using a Foswiki-generated Self-signed certificate
# or a certificate installed from a Foswiki-generated CSR, this field is automatically
...
# **BOOLEAN DISPLAY_IF="{EnableEmail} && /^Net::SMTP/.test({Email}{MailMethod})"**
...
# If no do not specify this or {Email}{SSLCaFile}, system defaults will be used.
...
# you trust.  The CRLs may be specified in a separate file {Email}{SSLCrlFile}, or may
# in {Email}{SSLCaPath}.
...
# <p>The plugins were discovered by searching the <code>@INC</code>
