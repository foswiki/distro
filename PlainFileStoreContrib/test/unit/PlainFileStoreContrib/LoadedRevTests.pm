...
sub test_no_history {
    ...
    my $f;
    open( $f, '>', "$Foswiki::cfg{DataDir}/$this->{test_web}/NoHistory.txt" )
      || return;
    print $f <<WHEE;
...
WHEE
    close($f);
    my ($topicObject) =
      Foswiki::Func::readTopic( $this->{test_web}, "NoHistory" );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );

    $topicObject->finish();
    $topicObject =
      $this->getUnloadedTopicObject( $this->{test_web}, "NoHistory" );
    $topicObject->load(3);
