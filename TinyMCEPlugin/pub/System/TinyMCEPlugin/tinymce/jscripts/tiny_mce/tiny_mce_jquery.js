(function(win){var whiteSpaceRe=/^\s*|\s*$/g,undefined;win.tinymce=win.tinyMCE={majorVersion:'3',minorVersion:'3b2',releaseDate:'2010-02-04',_init:function(){var t=this,d=document,na=navigator,ua=na.userAgent,i,nl,n,base,p,v;t.isOpera=win.opera&&opera.buildNumber;t.isWebKit=/WebKit/.test(ua);t.isIE=!t.isWebKit&&!t.isOpera&&(/MSIE/gi).test(ua)&&(/Explorer/gi).test(na.appName);t.isIE6=t.isIE&&/MSIE [56]/.test(ua);t.isGecko=!t.isWebKit&&/Gecko/.test(ua);t.isMac=ua.indexOf('Mac')!=-1;t.isAir=/adobeair/i.test(ua);if(win.tinyMCEPreInit){t.suffix=tinyMCEPreInit.suffix;t.baseURL=tinyMCEPreInit.base;t.query=tinyMCEPreInit.query;return;}
...
return 0;}}}
return 1;},trim:function(s){return(s?''+s:'').replace(whiteSpaceRe,'');},create:function(s,p){var t=this,sp,ns,cn,scn,c,de=0;s=/^((static) )?([\w.]+)(:([\w.]+))?/.exec(s);cn=s[3].match(/(^|\.)(\w+)$/i)[2];ns=t.createNS(s[3].replace(/\.\w+$/,''));if(ns[cn])
...
return u+v;return u.replace('#',v+'#');}};tinymce._init();})(window);(function($,tinymce){var is=tinymce.is,attrRegExp=/^(href|src|style)$/i,undefined;if(!$)
return alert("Load jQuery first!");tinymce.$=$;tinymce.adapter={patchEditor:function(editor){var fn=$.fn;function css(name,value){var self=this;if(value)
self.removeAttr('_mce_style');return fn.css.apply(self,arguments);};function attr(name,value){var self=this;if(attrRegExp.test(name)){if(value!==undefined){self.each(function(i,node){editor.dom.setAttrib(node,name,value);});return self;}else
...
re=new RegExp("(^|\\s+)"+c+"(\\s+|$)","g");v=e.className.replace(re,' ');v=tinymce.trim(v!=' '?v:'');e.className=v;if(!v)
e.removeAttribute('class');return v;}
...
if(x){h=h.replace(/<p ([^>]+)>|<p>/ig,'<div $1 _mce_tmp="1">');h=h.replace(/<\/p>/g,'</div>');set();if(t.settings.fix_ie_paragraphs){nl=e.getElementsByTagName("DIV");for(i=nl.length-1;i>=0;i--){n=nl[i];if(n._mce_tmp){p=t.doc.createElement('p');n.cloneNode(false).outerHTML.replace(/([a-z0-9\-_]+)=/gi,function(a,b){var v;if(b!=='_mce_tmp'){v=n.getAttribute(b);if(!v&&b==='class')
...
return v||s;}
return s;},encode:function(str){return(''+str).replace(encodeCharsRe,function(chr){return encodedChars[chr];});},insertAfter:function(n,r){var t=this;r=t.get(r);return this.run(n,function(n){var p,ns;p=r.parentNode;ns=r.nextSibling;if(ns)
...
tinymce.removeUnload(t.destroy);},createRng:function(){var d=this.doc;return d.createRange?d.createRange():new tinymce.dom.Range(this);},nodeIndex:function(node,normalized){var idx=0,lastNode,nodeType;if(node){for(node=node.previousSibling,lastNode=node;node;node=node.previousSibling){nodeType=node.nodeType;if(normalized&&nodeType==3){if(node.nodeValue.length>0&&(lastNode.nodeType!=nodeType||lastNode.nodeValue.length===0))
idx++;}else
idx++;lastNode=node;}}
...
return how==CLONE?n.cloneNode(TRUE):n;n.parentNode.removeChild(n);};};ns.Range=Range;})(tinymce.dom);(function(){function Selection(selection){var t=this,invisibleChar='\uFEFF',range,lastIERng,dom=selection.dom;function compareRanges(rng1,rng2){if(rng1&&rng2){if(rng1.item&&rng2.item&&rng1.item(0)===rng2.item(0))
return 1;if(rng1.isEqual&&rng2.isEqual&&rng2.isEqual(rng1))
return 1;}
return 0;};function getRange(){var ieRange=selection.getRng(),domRange=dom.createRng(),ieRange2,element,collapsed,isMerged;element=ieRange.item?ieRange.item(0):ieRange.parentElement();if(element.ownerDocument!=dom.doc)
...
ieRange2=ieRange.duplicate();collapsed=selection.isCollapsed();ieRange.collapse();ieRange.pasteHTML('<span id="_mce_start" style="display:none;line-height:0">\uFEFF</span>');if(!collapsed){ieRange2.collapse(false);ieRange2.pasteHTML('<span id="_mce_end" style="display:none;line-height:0">\uFEFF</span>');}
function setEndPoint(start){var container,offset,marker,sibling;marker=dom.get('_mce_'+(start?'start':'end'));sibling=marker.previousSibling;if(sibling&&sibling.nodeType==3){container=sibling;offset=container.nodeValue.length;dom.remove(marker);sibling=container.nextSibling;if(sibling&&sibling.nodeType==3){isMerged=true;container.appendData(sibling.nodeValue);dom.remove(sibling);}}else{sibling=marker.nextSibling;if(sibling&&sibling.nodeType==3){container=sibling;offset=0;}else{if(sibling)
offset=dom.nodeIndex(sibling)-1;else
offset=dom.nodeIndex(marker);container=marker.parentNode;}
dom.remove(marker);}
if(start)
domRange.setStart(container,offset);if(!start||collapsed)
domRange.setEnd(container,offset);};setEndPoint(true);if(!collapsed)
setEndPoint(false);if(isMerged)
t.addRange(domRange);return domRange;};this.addRange=function(rng){var ieRng,ieRng2,doc=selection.dom.doc,body=doc.body,startPos,endPos,sc,so,ec,eo,marker,lastIndex,skipStart,skipEnd;this.destroy();sc=rng.startContainer;so=rng.startOffset;ec=rng.endContainer;eo=rng.endOffset;ieRng=body.createTextRange();if(sc==doc||ec==doc){ieRng=body.createTextRange();ieRng.collapse();ieRng.select();return;}
if(sc.nodeType==1&&sc.hasChildNodes()){lastIndex=sc.childNodes.length-1;if(so>lastIndex){skipStart=1;sc=sc.childNodes[lastIndex];}else
sc=sc.childNodes[so];if(sc.nodeType==3)
so=0;}
if(ec.nodeType==1&&ec.hasChildNodes()){lastIndex=ec.childNodes.length-1;if(eo==0){skipEnd=1;ec=ec.childNodes[0];}else{ec=ec.childNodes[Math.min(lastIndex,eo-1)];if(ec.nodeType==3)
eo=ec.nodeValue.length;}}
if(sc==ec&&sc.nodeType==1){if(/^(IMG|TABLE)$/.test(sc.nodeName)&&so!=eo){ieRng=body.createControlRange();ieRng.addElement(sc);}else{ieRng=body.createTextRange();if(!sc.hasChildNodes()&&sc.canHaveHTML)
...
if(so==eo)
ieRng.collapse(eo<=rng.endContainer.childNodes.length-1);ieRng.select();ieRng.scrollIntoView();return;}
ieRng=body.createTextRange();marker=doc.createElement('span');marker.innerHTML=' ';if(sc.nodeType==3){if(skipStart)
dom.insertAfter(marker,sc);else
sc.parentNode.insertBefore(marker,sc);ieRng.moveToElementText(marker);marker.parentNode.removeChild(marker);ieRng.move('character',so);}else{ieRng.moveToElementText(sc);if(skipStart)
ieRng.collapse(false);}
if(sc==ec&&sc.nodeType==3){ieRng.moveEnd('character',eo-so);ieRng.select();ieRng.scrollIntoView();return;}
ieRng2=body.createTextRange();if(ec.nodeType==3){ec.parentNode.insertBefore(marker,ec);ieRng2.moveToElementText(marker);marker.parentNode.removeChild(marker);ieRng2.move('character',eo);ieRng.setEndPoint('EndToStart',ieRng2);}else{ieRng2.moveToElementText(ec);ieRng2.collapse(!!skipEnd);ieRng.setEndPoint('EndToEnd',ieRng2);}
ieRng.select();ieRng.scrollIntoView();};this.getRangeAt=function(){if(!range||!compareRanges(lastIERng,selection.getRng())){range=getRange();lastIERng=selection.getRng();}
...
endSelection();started=1;startRng=rngFromPoint(e.x,e.y);if(startRng){dom.bind(doc,'mouseup',endSelection);dom.bind(doc,'mousemove',selectionChange);startRng.select();}}});})();}};tinymce.dom.TridentSelection=Selection;})();(function(tinymce){var each=tinymce.each,DOM=tinymce.DOM,isIE=tinymce.isIE,isWebKit=tinymce.isWebKit,Event;tinymce.create('tinymce.dom.EventUtils',{EventUtils:function(){this.inits=[];this.events=[];},add:function(o,n,f,s){var cb,t=this,el=t.events,r;if(n instanceof Array){r=[];each(n,function(n){r.push(t.add(o,n,f,s));});return r;}
...
wa=' ';s.getInner=true;s.content=t.isCollapsed()?'':wb+t.serializer.serialize(e,s)+wa;t.onGetContent.dispatch(t,s);return s.content;},setContent:function(h,s){var t=this,r=t.getRng(),c,d=t.win.document;s=s||{format:'html'};s.set=true;h=s.content=t.dom.processHTML(h);t.onBeforeSetContent.dispatch(t,s);h=s.content;if(r.insertNode){h+='<span id="__caret">_</span>';if(r.startContainer==d&&r.endContainer==d){d.body.innerHTML=h;}else{r.deleteContents();r.insertNode(t.getRng().createContextualFragment(h));}
...
return e.firstChild||e;return e;}else{e=r.startContainer;if(e.nodeType==1&&e.hasChildNodes())
e=e.childNodes[Math.min(e.childNodes.length-1,r.startOffset)];if(e&&e.nodeType==3)
return e.parentNode;return e;}},getEnd:function(){var t=this,r=t.getRng(),e,eo;if(isIE){if(r.item)
...
index=i;});return index;};if(type==2){function getLocation(){var rng=t.getRng(true),root=dom.getRoot(),bookmark={};function getPoint(rng,start){var indexes=[],node,lastIdx,container=rng[start?'startContainer':'endContainer'],offset=rng[start?'startOffset':'endOffset'],exclude,point={};if(container.nodeType==1&&container.hasChildNodes()){lastIdx=container.childNodes.length-1;point.exclude=(start&&offset>lastIdx)||(!start&&offset==0);if(!start&&offset)
offset--;container=container.childNodes[offset>lastIdx?lastIdx:offset];if(container.nodeType==3)
offset=start?0:container.nodeValue.length;}
if(container.nodeType==3){if(normalized){for(node=container.previousSibling;node&&node.nodeType==3;node=node.previousSibling)
offset+=node.nodeValue.length;}
point.offset=offset;}
for(;container&&container!=root;container=container.parentNode)
indexes.push(t.dom.nodeIndex(container,normalized));point.indexes=indexes;return point;};bookmark.start=getPoint(rng,true);if(!t.isCollapsed())
...
t.moveToBookmark({id:id,keep:1});return{id:id};},moveToBookmark:function(bookmark){var t=this,dom=t.dom,marker1,marker2,rng,root;if(t.tridentSel)
t.tridentSel.destroy();if(bookmark){if(bookmark.start){rng=dom.createRng();root=dom.getRoot();function setEndPoint(start){var point=bookmark[start?'start':'end'],i,node,offset;if(point){for(node=root,i=point.indexes.length-1;i>=0;i--)
node=node.childNodes[point.indexes[i]]||node;if(start){if(node.nodeType==3&&point.offset)
rng.setStart(node,point.offset);else{if(point.exclude)
rng.setStartAfter(node);else
rng.setStartBefore(node);}}else{if(node.nodeType==3&&point.offset)
rng.setEnd(node,point.offset);else{if(point.exclude)
rng.setEndBefore(node);else
rng.setEndAfter(node);}}}};setEndPoint(true);setEndPoint();t.setRng(rng);}else if(bookmark.id){rng=dom.createRng();function restoreEndPoint(suffix){var marker=dom.get(bookmark.id+'_'+suffix),node,idx,next,prev,keep=bookmark.keep;if(marker){node=marker.parentNode;if(suffix=='start'){if(!keep){idx=dom.nodeIndex(marker);if(idx>0)
idx++;}else{node=marker;idx=1;}
rng.setStart(node,idx);rng.setEnd(node,idx);}else{if(!keep){idx=dom.nodeIndex(marker);}else{node=marker;idx=1;}
rng.setEnd(node,idx);}
...
dom.remove(marker,1);if(prev&&next&&prev.nodeType==next.nodeType&&prev.nodeType==3){idx=prev.nodeValue.length;prev.appendData(next.nodeValue);dom.remove(next);if(suffix=='start'){rng.setStart(prev,idx);rng.setEnd(prev,idx);}else
rng.setEnd(prev,idx);}}}};restoreEndPoint('start');restoreEndPoint('end');t.setRng(rng);}else if(bookmark.name){t.select(dom.select(bookmark.name)[bookmark.index]);}else if(bookmark.rng)
...
try{r.select();}catch(ex){}}},setNode:function(n){var t=this;t.setContent(t.dom.getOuterHTML(n));return n;},getNode:function(){var t=this,rng=t.getRng(),sel=t.getSel(),elm;if(!isIE){if(!rng)
...
o.content=h;},_serializeNode:function(n,inner){var t=this,s=t.settings,w=t.writer,hc,el,cn,i,l,a,at,no,v,nn,ru,ar,iv,closed,keep,type;if(!s.node_filter||s.node_filter(n)){switch(n.nodeType){case 1:if(n.hasAttribute?n.hasAttribute('_mce_bogus'):n.getAttribute('_mce_bogus'))
...
keep=1;}
if(isIE){if(n.scopeName!=='HTML'&&n.scopeName!=='html')
nn=n.scopeName+':'+nn;}
if(nn.indexOf('mce:')===0)
...
scriptLoadedCallbacks[url]=[];scriptLoadedCallbacks[url].push({func:callback,scope:scope||this});}};this.loadQueue=function(callback,scope){this.loadScripts(queue,callback,scope);};this.loadScripts=function(scripts,callback,scope){function execScriptLoadedCallbacks(url){tinymce.each(scriptLoadedCallbacks[url],function(callback){callback.func.call(callback.scope);});scriptLoadedCallbacks[url]=undefined;};queueLoadedCallbacks.push({func:callback,scope:scope||this});(function loadScripts(){var loadingScripts=tinymce.grep(scripts);scripts.length=0;tinymce.each(loadingScripts,function(url){if(states[url]==LOADED){execScriptLoadedCallbacks(url);return;}
if(states[url]!=LOADING){states[url]=LOADING;loading++;loadScript(url,function(){states[url]=LOADED;loading--;execScriptLoadedCallbacks(url);loadScripts();});}});if(!loading){tinymce.each(queueLoadedCallbacks,function(callback){callback.func.call(callback.scope);});queueLoadedCallbacks.length=0;}})();};};tinymce.ScriptLoader=new tinymce.dom.ScriptLoader();})(tinymce);tinymce.dom.TreeWalker=function(start_node,root_node){var node=start_node;function findSibling(node,start_name,sibling_name,shallow){var sibling,parent;if(node){if(!shallow&&node[start_name])
...
callback(siblings);walkBoundary(endContainer,endPoint);};};})(tinymce);(function(tinymce){var DOM=tinymce.DOM,is=tinymce.is;tinymce.create('tinymce.ui.Control',{Control:function(id,s){this.id=id;this.settings=s=s||{};this.rendered=false;this.onRender=new tinymce.util.Dispatcher(this);this.classPrefix='';this.scope=s.scope||this;this.disabled=0;this.active=0;},setDisabled:function(s){var e;if(s!=this.disabled){e=DOM.get(this.id);if(e&&this.settings.unavailable_prefix){if(s){this.prevTitle=e.title;e.title=this.settings.unavailable_prefix+": "+e.title;}else
...
if(s.auto_focus){setTimeout(function(){var ed=tinymce.get(s.auto_focus);ed.selection.select(ed.getBody(),1);ed.selection.collapse(1);ed.getWin().focus();},100);}},1);e=null;},focus:function(sf){var oed,t=this,ce=t.settings.content_editable;if(!sf){if(!ce&&(!isIE||t.selection.getNode().ownerDocument!=t.getDoc()))
t.getWin().focus();}
...
o[tr(v[0])]=tr(v);});}else
o=v;return o;}
return v;},nodeChanged:function(o){var t=this,s=t.selection,n=s.getNode()||t.getBody();if(t.initialized){o=o||{};n=isIE&&n.ownerDocument!=t.getDoc()?t.getBody():n;o.parents=[];t.dom.getParent(n,function(node){if(node.nodeName=='BODY')
...
if(tinymce.isWebKit){t.onClick.add(function(ed,e){e=e.target;if(e.nodeName=='IMG')
...
addUndo();});t.onKeyDown.add(function(ed,e){if((e.keyCode>=33&&e.keyCode<=36)||(e.keyCode>=37&&e.keyCode<=40)||e.keyCode==13||e.keyCode==45){if(t.undoManager.typing)
addUndo();return;}
...
if(failed||!doc.queryCommandEnabled(command)){if(tinymce.isGecko){editor.windowManager.confirm(editor.getLang('clipboard_msg'),function(state){if(state)
...
return null;}
if(s.custom_undo_redo_levels){if(t.data.length>s.custom_undo_redo_levels){for(i=0;i<t.data.length-1;i++)
t.data[i]=t.data[i+1];t.data.length--;t.index=t.data.length;}}
if(s.custom_undo_redo_restore_selection)
l.bookmark=b=l.bookmark||ed.selection.getBookmark(2,true);if(t.index<t.data.length-1){if(t.index==0)
t.data=[];else
t.data.length=t.index+1;}
t.data.push(l);t.index=t.data.length-1;t.onAdd.dispatch(t,l);ed.isNotDirty=0;return l;},undo:function(){var t=this,ed=t.editor,l=l,i;if(t.typing){t.add();t.typing=0;}
if(t.index>0){l=t.data[--t.index];ed.setContent(l.content,{format:'raw'});ed.selection.moveToBookmark(l.bookmark);t.onUndo.dispatch(t,l);}
...
if(!isIE&&s.force_p_newlines){ed.onKeyPress.add(function(ed,e){if(e.keyCode==13&&!e.shiftKey&&!t.insertPara(e))
Event.cancel(e);});if(isGecko){ed.onKeyDown.add(function(ed,e){if((e.keyCode==8||e.keyCode==46)&&!e.shiftKey)
t.backspaceDelete(e,e.keyCode==8);});}}
if(tinymce.isWebKit){function insertBr(ed){var rng=selection.getRng(),br;rng.insertNode(br=dom.create('br'));rng.setStartAfter(br);rng.setEndAfter(br);selection.setRng(rng);if(selection.getSel().focusNode==br.previousSibling){selection.select(dom.insertAfter(dom.doc.createTextNode('\u00a0'),br));selection.collapse(TRUE);}
ed.getWin().scrollTo(0,dom.getPos(selection.getRng().startContainer).y);};ed.onKeyPress.add(function(ed,e){if(e.keyCode==13&&(e.shiftKey||s.force_br_newlines)){insertBr(ed);Event.cancel(e);}});}
...
return cmd.func.call(scope||cmd.scope,ui,value,args);}});};tinymce.GlobalCommands=new CommandManager();})(tinymce);(function(tinymce){tinymce.Formatter=function(ed){var formats={},each=tinymce.each,dom=ed.dom,selection=ed.selection,TreeWalker=tinymce.dom.TreeWalker,rangeUtils=new tinymce.dom.RangeUtils(dom),isValid=ed.schema.isValid,isBlock=dom.isBlock,forcedRootBlock=ed.settings.forced_root_block,nodeIndex=dom.nodeIndex,INVISIBLE_CHAR='\uFEFF',FALSE=false,TRUE=true,undefined,caretHandler,pendingFormats;function getParents(node,selector){return dom.getParents(node,selector,dom.getRoot());};function resetPending(){if(!pendingFormats||pendingFormats.apply.length||pendingFormats.remove.length)
pendingFormats={apply:[],remove:[]};};ed.onMouseUp.add(resetPending);resetPending();function get(name){return name?formats[name]:formats;};function register(name,format){if(name){if(typeof(name)!=='string'){each(name,function(format,name){register(name,format);});}else{format=format.length?format:[format];each(format,function(format){if(format.deep===undefined)
format.deep=!format.selector;if(format.split===undefined)
format.split=!format.selector;if(format.remove===undefined&&format.selector)
format.remove='none';if(typeof(format.classes)==='string')
format.classes=format.classes.split(/\s+/);});formats[name]=format;}}};function apply(name,vars,node){var formatList=get(name),format=formatList[0],bookmark,rng,i;function moveStart(rng){var container=rng.startContainer,offset=rng.startOffset,walker,node;if(container.nodeType==1||container.nodeValue===""){walker=new TreeWalker(container.childNodes[offset]);for(node=walker.current();node;node=walker.next()){if(node.nodeType==3&&!isBlock(node.parentNode)&&!isWhiteSpaceNode(node)){rng.setStart(node,0);break;}}}
...
dom.addClass(elm,value);});}};function applyRngStyle(rng){var newWrappers=[],wrapName,wrapElm;wrapName=format.inline||format.block;wrapElm=dom.create(wrapName);setElementFormat(wrapElm);rangeUtils.walk(rng,function(nodes){var currentWrapElm;function process(node){var nodeName=node.nodeName.toLowerCase(),parentName=node.parentNode.nodeName.toLowerCase();if(isEq(nodeName,'br')){currentWrapElm=0;if(format.block)
...
if(format.selector){each(formatList,function(format){if(dom.is(node,format.selector))
setElementFormat(node,format);});return;}
...
count++;});return count;};function mergeStyles(node){var child,clone;each(node.childNodes,function(node){if(node.nodeType==1&&!isBookmarkNode(node)){child=node;return FALSE;}});if(child&&matchName(child,format)){clone=child.cloneNode(FALSE);setElementFormat(clone);dom.replace(clone,node,TRUE);dom.remove(child,1);return TRUE;}};childCount=getChildCount(node);if(childCount===0){dom.remove(node,1);return;}
if(format.inline||format.wrapper){if(!format.exact&&childCount===1){if(mergeStyles(node))
return;}
each(formatList,function(format){each(dom.select(format.inline,node),function(child){removeFormat(format,vars,child,format.exact?child:null);});});dom.getParent(node.parentNode,function(parent){if(matchNode(parent,name,vars)){dom.remove(node,1);node=0;return TRUE;}});if(node){node=mergeSiblings(getNonWhiteSpaceSibling(node),node);node=mergeSiblings(node,getNonWhiteSpaceSibling(node,TRUE));}}});};if(format){if(node){rng=dom.createRng();rng.setStartBefore(node);rng.setEndAfter(node);applyRngStyle(rng);}else{if(!selection.isCollapsed()||!format.inline){bookmark=selection.getBookmark();applyRngStyle(expandRng(selection.getRng(TRUE),formatList));selection.moveToBookmark(bookmark);selection.setRng(moveStart(selection.getRng(TRUE)));ed.nodeChanged();}else
performCaretAction('apply',name,vars);}}};function remove(name,vars,node){var formatList=get(name),format=formatList[0],bookmark,i,rng;function process(node){var children;children=tinymce.grep(node.childNodes);each(formatList,function(format){if(removeFormat(format,vars,node,node))
return FALSE;});if(format.deep){each(formatList,function(format){each(children,function(node){process(node);});});}};function findFormatRoot(container){var formatRoot;each(getParents(container.parentNode).reverse(),function(parent){if(!formatRoot&&parent.id!='_start'&&parent.id!='_end'){if(!isBlock(parent)&&matchNode(parent,name,vars))
...
return container;};function splitToFormatRoot(container){return wrapAndSplit(findFormatRoot(container),container,container,true);};function unwrap(start){var node=dom.get(start?'_start':'_end'),out=node[start?'firstChild':'lastChild'];dom.remove(node,1);return out;};function removeRngStyle(rng){var startContainer,endContainer;rng=expandRng(rng,formatList,TRUE);if(format.split){startContainer=getContainer(rng,TRUE);endContainer=getContainer(rng);if(startContainer!=endContainer){startContainer=wrap(startContainer,'span',{id:'_start',_mce_type:'bookmark'});endContainer=wrap(endContainer,'span',{id:'_end',_mce_type:'bookmark'});splitToFormatRoot(startContainer);splitToFormatRoot(endContainer);startContainer=unwrap(TRUE);endContainer=unwrap();}else
...
if(!selection.isCollapsed()||!format.inline){bookmark=selection.getBookmark();removeRngStyle(selection.getRng(TRUE));selection.moveToBookmark(bookmark);ed.nodeChanged();}else
performCaretAction('remove',name,vars);};function toggle(name,vars,node){if(match(name,vars,node))
remove(name,vars,node);else
apply(name,vars,node);};function matchNode(node,name,vars){var formatList=get(name),format,i,classes;function matchItems(node,format,item_name){var key,value,items=format[item_name],i;if(items){if(items.length===undefined){for(key in items){if(items.hasOwnProperty(key)){if(item_name==='attributes')
value=dom.getAttrib(node,key);else
value=getStyle(node,key);if(!isEq(value,replaceVars(items[key],vars)))
return;}}}else{for(i=0;i<items.length;i++){if(item_name==='attributes'?dom.getAttrib(node,items[i]):getStyle(node,items[i]))
return TRUE;}}}
return TRUE;};if(formatList&&node){for(i=0;i<formatList.length;i++){format=formatList[i];if(matchName(node,format)&&matchItems(node,format,'attributes')&&matchItems(node,format,'styles')){if(classes=format.classes){for(i=0;i<classes.length;i++){if(!dom.hasClass(node,classes[i]))
return;}}
return TRUE;}}}};function match(name,vars,node){var startNode,i;function matchParents(node){node=dom.getParent(node,function(node){return!!matchNode(node,name,vars);});return matchNode(node,name,vars);};if(node)
...
return TRUE;}
return FALSE;};function canApply(name){var formatList=get(name),startNode,parents,i,x,selector;if(formatList){startNode=selection.getStart();parents=getParents(startNode);for(x=formatList.length-1;x>=0;x--){selector=formatList[x].selector;if(!selector)
return TRUE;for(i=parents.length-1;i>=0;i--){if(dom.is(parents[i],selector))
return TRUE;}}}
return FALSE;};tinymce.extend(this,{get:get,register:register,apply:apply,remove:remove,toggle:toggle,match:match,matchNode:matchNode,canApply:canApply});function matchName(node,format){if(isEq(node,format.inline))
...
if(format[0].selector&&format[0].expand!==FALSE){function findSelectorEndPoint(container,sibling_name){var parents,i,y;if(container.nodeType==3&&container.nodeValue.length==0&&container[sibling_name])
...
break;}}
return node||container;};startContainer=findBlockEndPoint(startContainer,'previousSibling');endContainer=findBlockEndPoint(endContainer,'nextSibling');if(!isBlock(startContainer))
...
if(name=="class")
node.removeAttribute('className');node.removeAttribute(name);}});each(format.classes,function(value){value=replaceVars(value,vars);if(!compare_node||dom.hasClass(compare_node,value))
...
rootBlockElm=0;});}}}}
dom.remove(node,1);};function getNonWhiteSpaceSibling(node,next,inc){if(node){next=next?'nextSibling':'previousSibling';for(node=inc?node:node[next];node;node=node[next]){if(node.nodeType==1||!isWhiteSpaceNode(node))
...
return FALSE;function getAttribs(node){var attribs={};each(dom.getAttribs(node),function(attr){var name=attr.nodeName.toLowerCase();if(name.indexOf('_')!==0&&name!=='class'&&name!=='style')
...
function unbind(){if(caretHandler){each(events,function(event){ed[event].remove(caretHandler);});caretHandler=0;}};function perform(caret_node){each(pendingFormats.apply.reverse(),function(item){apply(item.name,item.vars,caret_node);});each(pendingFormats.remove.reverse(),function(item){remove(item.name,item.vars,caret_node);});dom.remove(caret_node,1);resetPending();};function isMarker(node){return node.face==marker||node.style.fontFamily==marker;};unbind();doc.execCommand('FontName',false,marker);each(dom.select('font,span',selectedNode),function(node){var bookmark;if(isMarker(node)){bookmark=selection.getBookmark();perform(node);selection.moveToBookmark(bookmark);ed.nodeChanged();selectedNode=0;}});if(selectedNode){caretHandler=function(ed,e){each(dom.select('font,span',selectedNode),function(node){var bookmark,textNode;if(node.face==marker||node.style.fontFamily==marker){textNode=node.firstChild;perform(node);rng=dom.createRng();rng.setStart(textNode,textNode.nodeValue.length);rng.setEnd(textNode,textNode.nodeValue.length);selection.setRng(rng);ed.nodeChanged();unbind();}});if(e.type=='keyup'){unbind();resetPending();}};each(events,function(event){ed[event].addToTop(caretHandler);});}}};})(tinymce);tinymce.onAddEditor.add(function(tinymce,ed){var filters,fontSizes,dom,settings=ed.settings;if(settings.inline_styles){fontSizes=tinymce.explode(settings.font_size_style_values);function replaceWithSpan(node,styles){dom.replace(dom.create('span',{style:styles}),node,1);};filters={font:function(dom,node){replaceWithSpan(node,{backgroundColor:node.style.backgroundColor,color:node.color,fontFamily:node.face,fontSize:fontSizes[parseInt(node.size)-1]});},u:function(dom,node){replaceWithSpan(node,{textDecoration:'underline'});},strike:function(dom,node){replaceWithSpan(node,{textDecoration:'line-through'});}};function convert(editor,params){dom=editor.dom;if(settings.convert_fonts_to_spans){tinymce.each(dom.select('font,u,strike',params.node),function(node){filters[node.nodeName.toLowerCase()](ed.dom,node);});}};ed.onPreProcess.add(convert);ed.onInit.add(function(){ed.selection.onSetContent.add(convert);});}});