...
sub initBinary {
    ...
    my ( $rcsOutput, $exit ) =
      $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{initBinaryCmd},
        FILENAME => $this->{file} );
    if ($exit) {
        throw Error::Simple( $TWiki::cfg{RCS}{initBinaryCmd} . ' of '
...
sub initText {
    ...
    my ( $rcsOutput, $exit ) =
      $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{initTextCmd},
        FILENAME => $this->{file} );
    if ($exit) {
        $rcsOutput ||= '';
        throw Error::Simple( $TWiki::cfg{RCS}{initTextCmd} . ' of '
              . $this->hidePath( $this->{file} )
              . ' failed: '
              . $rcsOutput );
    }
...
sub _deleteRevision {
    ...
    my ( $rcsOut, $exit ) =
      $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{unlockCmd},
        FILENAME => $this->{file} );
    ...
    if ($exit) {
        throw Error::Simple( $TWiki::cfg{RCS}{delRevCmd} . ' of '
              . $this->hidePath( $this->{file} )
              . ' failed: '
              . $rcsOut );
    }
    ...
    if ($exit) {
        throw Error::Simple( $TWiki::cfg{RCS}{coCmd} . ' of '
              . $this->hidePath( $this->{file} )
              . ' failed: '
              . $rcsOut );
    }
...
sub getRevision {
    ...
    if ( $TWiki::cfg{RCS}{coMustCopy} ) {
        ...
        my ( $tmp, $status ) =
          $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{tmpBinaryCmd},
            FILENAME => $tmpRevFile );
        $file = $tmpfile;
...
sub numRevisions {
    ...
    my ( $rcsOutput, $exit ) =
      $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{histCmd},
        FILENAME => $this->{rcsFile} );
    if ($exit) {
        throw Error::Simple( 'RCS: '
              . $TWiki::cfg{RCS}{histCmd} . ' of '
              . $this->hidePath( $this->{rcsFile} )
              . ' failed: '
              . $rcsOutput );
    }
...
sub _lock {
    ...
    my ( $rcsOutput, $exit ) =
      $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{lockCmd},
        FILENAME => $this->{file} );
    ...
    if ($exit) {
        ...
        if ( ( time - ( stat( $this->{rcsFile} ) )[9] ) > 3600 ) {
            ...
            $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{breaklockCmd},
                FILENAME => $this->{file} );
            ( $rcsOutput, $exit ) =
              $TWiki::sandbox->sysCommand( $TWiki::cfg{RCS}{lockCmd},
                FILENAME => $this->{file} );
