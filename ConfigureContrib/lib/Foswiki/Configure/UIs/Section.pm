...
sub renderHtml {
    ...
    my $class = $section->hasDeep('EXPERT') ? 'configureExpert' : '';
    ...
    if ( $section->{_parent} ) {
        if ( $section->{_parent}->{TABS} ) {
            ...
            $section->{_parent}->{controls} ||=
              new Foswiki::Configure::GlobalControls(
                $this->makeID( $section->{_parent}->{headline} || '' ) );

            $section->{_parent}->{controls}
              ->openTab( $id, $depth, $section, $section->{headline},
                $sectionErrors, $sectionWarnings );

            $id = $section->{_parent}->{controls}->sectionId($id);
...
sub _renderValues {
    ...
    my $expertCount = 0;
    my $infoCount   = 0;
    foreach my $item ( @{ $section->{children} } ) {
        next unless $item->isa('Foswiki::Configure::Value');
