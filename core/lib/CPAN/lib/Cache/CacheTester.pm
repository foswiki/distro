...
sub _test_one {
    ...
    ( $fetched_value eq $value )
      ? $self->ok()
      : $self->not_ok('$fetched_value eq $value');
    ...
    ( not defined $fetched_removed_value )
      ? $self->ok()
      : $self->not_ok('not defined $fetched_removed_value');
}
...
sub _test_two {
    ...
    if (    ( $fetched_value_list_ref->[0] eq 'One' )
        and ( $fetched_value_list_ref->[1] eq 'Two' )
        and ( $fetched_value_list_ref->[2] eq 'Three' ) )
    {
        $self->ok();
    }
    else {
...
sub _test_three {
    ...
    ( defined $fetched_cache )
      ? $self->ok()
      : $self->not_ok('defined $fetched_cache');
...
sub _test_four {
    ...
    ( not defined $fetched_expired_value )
      ? $self->ok()
      : $self->not_ok('not defined $fetched_expired_value');
}
...
sub _test_six {
    ...
    ( not defined $fetched_cleared_value )
      ? $self->ok()
      : $self->not_ok('not defined $fetched_cleared_value');
}
...
sub _test_eight {
    ...
    ( $pre_purge_size > $empty_size )
      ? $self->ok()
      : $self->not_ok('$pre_purge_size > $empty_size');
    ...
    ( $post_purge_size == $empty_size )
      ? $self->ok()
      : $self->not_ok('$post_purge_size == $empty_size');
}
...
sub _test_nine {
    ...
    my $cache2 = $cache1->new()
      or croak("Couldn't construct new cache");
...
sub _test_ten {
    ...
    ( $full_size > 0 ) ? $self->ok() : $self->not_ok('$full_size > 0');
...
sub _test_eleven {
    ...
    ( not defined $purged_object )
      ? $self->ok()
      : $self->not_ok('not defined $purged_object');
}
...
sub _test_twelve {
    ...
    my $class = ref $cache
      or croak("Couldn't get ref \$cache");
...
sub _test_fourteen {
    ...
    ( $arrays_equal == 1 ) ? $self->ok() : $self->not_ok('$arrays_equal == 1');
}
...
sub _test_fifteen {
    ...
    ( not defined $fetched_expired_object )
      ? $self->ok()
      : $self->not_ok('not defined $fetched_expired_object');
...
sub _test_sixteen {
    ...
    eval { $cache = $cache->new( { 'auto_purge_interval' => $expires_in } ); };
